...
def run_backtest(days: int = 252, markets_per_day: int = 40,
    ...
    for day_index, markets in gen.stream(days):
        broker.start_new_day()
        # Rank the day's opportunities by estimated edge and take the best-first,
        # so the stake cap is spent on the strongest bets rather than the first
        # ones seen.
        intents = []
        for m in markets:
            model_prob = forecaster.estimate(m)
        ...
        for intent, m in intents:
            if broker._breaker_blocking_new_bets():
                # A breaker (drawdown / daily-loss) is account-wide: once it
                # trips, stop opening for the day. This is the guardrails
            ...
            try:
                ...
                # This one bet didn't fit (cash spent on higher-edge bets, or the
                # cap yields zero here). Skip it and keep trying lower-edge ones —
                # a single unplaceable bet must NOT cancel the whole day.
                continue

        # Settle everything opened today at day's end (buy-and-hold-to-resolution
        # is this strategy's whole lifecycle).
        for m in opened:
            broker.settle(m.ticker, yes_won=m.outcome)

        equity_curve.append(broker.bankroll)
        if verbose and (day_index % max(1, days // 10) == 0):
    ...
    return BacktestResult(
        ...
        total_fees=broker.total_fees_paid, max_drawdown=max_dd,
